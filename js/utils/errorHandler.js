...
export const handleError = (error, type = ErrorTypes.NETWORK) => {
    ...
    // Update spinner if it's visible
    const spinner = document.getElementById('loading-spinner');
    if (spinner && spinner.style.display === 'block') {
        const spinnerMessage = document.querySelector('.spinner-message');
        if (spinnerMessage) {
            spinnerMessage.textContent = `Error: ${errorMessage}. Please try again.`;
            spinnerMessage.style.color = '#e74c3c'; // Red for error
            
            // Hide spinner after a short delay
            setTimeout(() => {
                spinner.style.display = 'none';
                if (spinnerMessage) {
                    spinnerMessage.style.color = ''; // Reset color
                }
            }, 3000);
            return; // Don't show another error message
        }
    }
